...
public final class SymbolTools {
    ...
    */
    ...
    public static List getNativeSpecifiers(Traversable t, Symbol symbol) {
        ...
        if (symbol.getArraySpecifiers() != null) {
...
/*
    // Checks if the two declarations are legal assuming it contains a symbol
    // with a same name.
    private static boolean
    isLegalDuplicateSymbol(Declaration d0, Declaration d1) {
        // TODO:
        // d0 and d1 should have the same specifiers except for "EXTERN".
        // only one of d0 and d1 can have initializers
        return true;
    }
*/
    ...
    */
    public static void addSymbols(SymbolTable table, Declaration decl) {
        ...
        for (int i = 0; i < ids_size; i++) {
            ...
            Declaration entry = symbol_table.get(id);
            if (entry != null && !(decl instanceof Procedure)) {
/*
                if (decl instanceof Enumeration ||
                    decl instanceof ClassDeclaration ||
                    isLegalDuplicateSymbol(entry, decl)) {
                    // These cases are legally possible in our symbol table
                    // management
                } else {
                    // Cetus does not stop in this case, but it needs to let
                    // the users know about this.
                    PrintTools.printlnStatus(0, "[WARNING]", "symbol", id,
                            "was already declared in", entry);
                }
*/
                continue;
            }
