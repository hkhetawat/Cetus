...
public final class Tools {
    ...
    */
    ...
    public static boolean containsClasses(Collection c, Class... types) {
        if (c != null) {
            for (Object o : c) {
    ...
    */
    public static void exit(int status) {
        if (exit_throws_exception) {
        	...
        	Driver.checkUpdate();
//        	Driver.t2 = new Thread2(0);
//        	Driver.t2.start(); //thread 2 for other things, like version check
        	//Driver.t2.interrupt(); //if thread 2 for checking new version has not finished after Cetus finished, interrupt it.
            System.exit(status);
        }
    }
    ...
    */
     ...
     * @param file output C file
     */
	public static void processIndent(File file){
		
		String fileString = file.toString();
		//System.out.println("[Tools] Processing indentation for file: "+fileString);
		int numOfLeftBrackets = 0;
		
		try {
			String tempFileString = fileString+".tmp";
			// Create the indented file with modified name under the same directory
			//System.out.println("[Tools] Created a temporary indentation file: "+tempFileString);
			PrintWriter out = new PrintWriter(new FileWriter(tempFileString));
			...
			file.delete();
			//System.out.println("[Tools] Deleted file: "+fileString);
			File tempFile = new File(tempFileString);
			tempFile.renameTo(file);
			//System.out.println("[Tools] Renamed the temporary indentation file: "+tempFileString);
			System.out.println("[Tools] Indentation process was successful for file: "+fileString);	
