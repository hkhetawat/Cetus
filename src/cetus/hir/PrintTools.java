...
//    private static int verbosity = 0;        
    ...
    */
    ...
    public static int getVerbosity() {
    	if (Driver.getOptionValue("verbosity")!=null)
    		return Integer.valueOf(Driver.getOptionValue("verbosity")).intValue();
    	else return 0;
    }
    ...
    * @param t the traversable object to be processed.
    */
    public static void removeVerboseParens(Traversable t) {
        DFIterator iter = new DFIterator(t);
        while (iter.hasNext()) {
            Traversable child = iter.next();
            if (child instanceof ArrayAccess) {
                for (Expression index : ((ArrayAccess)child).getIndices()) {
                    if (index != null) {
                        index.setParens(false);
                    }
                }
            } else if (child instanceof FunctionCall) {
                for (Expression arg : ((FunctionCall)child).getArguments()) {
                    if (arg != null) {
                        arg.setParens(false);
                    }
                }
            } else if (child instanceof IfStatement) {
                ((IfStatement)child).getControlExpression().setParens(false);
            } else if (child instanceof SwitchStatement) {
                ((SwitchStatement)child).getExpression().setParens(false);
            } else if (child instanceof Loop) {
                Expression condition = ((Loop)child).getCondition();
                if (condition != null) {
                    condition.setParens(false);
                }
                if (child instanceof ForLoop) {
                    Expression step = ((ForLoop)child).getStep();
                    if (step != null) {
