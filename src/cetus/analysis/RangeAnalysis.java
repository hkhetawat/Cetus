    ...
    private static void
        ...
        if ((Integer)node.getData("num-visits") < 2) {
            ...
            if (IRTools.containsUnary(to, UnaryOperator.DEREFERENCE)) {
                ...
                if (types == null ||
                    types.contains(Specifier.INT) ||
                    types.contains(Specifier.LONG)) {
                    direction = "discard";
                } else {
                    direction = "nochange"; // other types are ok.
                }
/* Disabled this code for two reasons:
   1. RangeAnalysis may result in transformation due to return normalization
      invoked by points-to analysis (same argument holds for points-to analysis
      , so somebody need TODO think about this.
   2. Conservative decision does not harm the overall result.
                if (types == null) {
                    direction = "discard";
