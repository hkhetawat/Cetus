...
/**
 * SplitOmpPRegion splits OpenMP parallel regions at each explicit/implicit synchronization point.
 * 
...
public class SplitOmpPRegion extends TransformPass {
	...
	public void splitParallelRegions()
	{
		...
		if( pRegionAnnots.size() == 0 ) {
			...
			for( OmpAnnotation omp_annot : pRegionAnnots ) {
				...
				atstmt.removeAnnotations(CudaAnnotation.class);
			}
	 ...
	 * @param orgPRegion Original parallel region where barriers in the barrier_list belong.
	 */
	private void createSubRegions(List<OmpAnnotation> barrier_list, HashMap old_map, Statement orgPRegion) {
		int barIndex = 0;
		int pBarIndex = 0;
				...
				while( barIndex > lastDeclIndex_plus1 ) {
					...
					if( list_size > 0 ) {
						...
						if( foundCompStmt ) {
							...
							////////////////////////////////////////////////////////////////////////////////
							//FIXME: If any shared variable in the CudaAnnotation is passed to a function //
							//as an argument, below copied CudaAnnotation may contain wrong variable name.//
							////////////////////////////////////////////////////////////////////////////////
							List<CudaAnnotation> cudaAnnots = orgPRegion.getAnnotations(CudaAnnotation.class);
							for( CudaAnnotation cAnnot : cudaAnnots ) {
								pRegion.annotate(cAnnot.clone());
							}
				...
				if( !lastSubRHandled ) {
					while( barIndex > lastDeclIndex_plus1 ) {
						...
						if( list_size > 0 ) {
							...
							// and this sub-region should not be extracted as a parallel    //
							// region.                                                      //
							// DEBUG: If this sub-region contains only a return statement,  //
							// this sub-region should not be extracted as a parallel region //
							// too.                                                         //
							//////////////////////////////////////////////////////////////////
							boolean foundCompStmt = false;
							for( Traversable temp : temp_list) {
	...
	public void ipCreateSubRegions(HashMap old_map, Statement region, Statement orgPRegion) {
		//////////////////////////////////////////////////////////
		...
		createSubRegions(barrier_list, old_map, orgPRegion);
		...
		for( FunctionCall call : calledFuncs ) {
			...
			if( (called_procedure == null) || 
				...
				TransformTools.updateAnnotationsInRegion(called_procedure, true);
			}
		}
		...
		//Update OmpAnnotations
		TransformTools.updateAnnotationsInRegion(region, true);
	}
