...
public class LoopCollapse {
	...
	{
		PrintTools.println("[handleSMVP] strt", 1);
		...
		for (OmpAnnotation annot : annot_lists)
		{
			...
			List tstmt_list = iter.getList(Statement.class);
			List stmt_list = new LinkedList();
			//Remove annotation statements.
			for( Object obj : tstmt_list ) {
			...
			List tinner_loop_stmt_list = inner_loop_iter.getList(Statement.class);
			List inner_loop_stmt_list = new LinkedList();
			//Remove annotation statements.
			for( Object obj : tinner_loop_stmt_list ) {
				if( !(obj instanceof AnnotationStatement) ) {
					inner_loop_stmt_list.add(obj);
				}
			}
			...
			PrintTools.println("row_array: " + row_array, 2);
			PrintTools.println("col_array: " + col_array, 2);
			PrintTools.println("mat_array: " + mat_array, 2);
			PrintTools.println("vec_array: " + vec_array, 2);
			PrintTools.println("out_array: " + out_array, 2);
			...
			if( !rowIDMap.containsKey(rowptrSym) ) {
				...
				str.append("CUDA_SAFE_CALL( cudaMemcpy( "+gpu_rowid+", "+rowid+", gpuBytes, cudaMemcpyHostToDevice) );\n");
				str.append("#endif \n");
		...
		PrintTools.println("[handleSMVP] done", 1);
		return isSMVP;
	}
