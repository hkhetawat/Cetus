...
public class O2GTranslator {
	...
	private static SymbolTable main_global_table;
	...
	private static boolean opt_MemTrOptOnLoops = false;
	private static boolean opt_doNotUseCUTIL = false;
	///////////////////////////////////////////
	...
	private static HashMap<Symbol, Statement> freedSymMap = null;
	...
	///////////////////////////////////////////////////////////////////////////////
	// HashSet containing local reduction array symbols, which will be allocated //
	// in the global memory as if threadprivate variables.                       //
	///////////////////////////////////////////////////////////////////////////////
	
	//////////////////////////////////////////////////////////////////////
	// This HashMap contains mapping of (CPU symbol, GPU symbol).       //
	// (This map is used if opt_globalGMalloc is true.)                 //
	// CPU symbol can be either VariableDeclarator or NestedDeclarator, //
	// but GPU symbol will be only VariableDeclarator, since in current //
	// implementation, OpenMP shared variables should be either scalar  //
	// or normal array variables (both are VariableDeclarator types).   //
	//////////////////////////////////////////////////////////////////////
	private static HashMap<Symbol, VariableDeclarator> gC2GMap = null;
	
	////////////////////////////////////////////////////////////////////
	// HashMap containing TranlationUnit to hidden comment indicating //
	// the last of CUDA-related headers.                              //
	////////////////////////////////////////////////////////////////////
	private static HashMap<TranslationUnit, Declaration> CUDAHeaderEndMap = null;
	
	////////////////////////////////////////////////////////////
	//Keep shared variable to CUDA constant variable mapping. //
	////////////////////////////////////////////////////////////
	private static HashMap<Symbol, Symbol> constMap = null;
	 ...
	 */
	public static void CUDAInitializer(Program prog) {
		...
		CUDAHeaderEndMap = new HashMap<TranslationUnit, Declaration>();
		
		/////////////////////////////////////////////////////////////////
		// Read command-line options and set corresponding parameters. //
		/////////////////////////////////////////////////////////////////
		String value = Driver.getOptionValue("doNotUseCUTIL");
		if( value != null ) {
			opt_doNotUseCUTIL = true;
			FunctionCall optNoCUTILCall = new FunctionCall(new NameID("printf"));
			optNoCUTILCall.addArgument(new StringLiteral("====> Generate CUDA code without CUTIL tools.\\n"));
			optPrintStmts.add( new ExpressionStatement(optNoCUTILCall) );
		}
		value = Driver.getOptionValue("useMallocPitch");
		if( value != null ) {
		...
		// If doNotUseCUTIL is on, this option is suppressed.
		if( (value != null) && (!opt_doNotUseCUTIL) ) {
...
/*			if( defaultBlockSize < 64 ) {
		...
		if( value != null ) {
			defaultSMemSize = Integer.valueOf(value).intValue();
		}
		...
		if( value != null ) {
			...
			gC2GMap = new HashMap<Symbol, VariableDeclarator>();
		...
		if( value != null ) {
			...
			shrdSclrCachingOnConstPrintCall.addArgument(new StringLiteral("====> Cache R/O shared scalar variables onto GPU constant memory.\\n"));
		...
		if( value != null ) {
			...
			shrdArryCachingOnConstPrintCall.addArgument(new StringLiteral("====> Cache R/O shared array variables onto GPU constant memory.\\n"));
			optPrintStmts.add( new ExpressionStatement(shrdArryCachingOnConstPrintCall) );
		}
		
		value = Driver.getOptionValue("disableCritical2ReductionConv");
		if( value != null ) {
			FunctionCall DisableC2RCall = new FunctionCall(new NameID("printf"));
			DisableC2RCall.addArgument(new StringLiteral("====> Critical-to-Reduction conversion is disabled.\\n"));
			optPrintStmts.add( new ExpressionStatement(DisableC2RCall) );
		}
		...
		if( value != null ) {
			...
			NoUnusedSymDelCall.addArgument(new StringLiteral("====> Do not remove unused symbols.\\n"));
		...
		for ( Traversable tt : program.getChildren() )
		{
			...
			if( !found_main ) {
				...
				if( found_main ) {
					...
					for(Declaration arg: (List<Declaration>)arglist) {
						...
						Expression expr = (Expression)((VariableDeclaration)arg).getDeclarator(0).getID();
					...
					if( arglist.size() > 0 ) {
						//////////////////////////////////////////////////////////////////////////////////
						//FIXME: CUT_DEVICE_INIT() is only for CUDA setting purpose, and added to       //
						//output cuda file if main has non-zero arguments. However, if main's arguments //
						//are not used for CUDA setting, but used for other purpose, this call should   //
						//not be inserted. For now, inserting this call is disabled. Instead, the next  //
						//code annoatation is used for CUDA device setting.                             //
						//////////////////////////////////////////////////////////////////////////////////
						/*
					...
					StringBuilder istr = new StringBuilder(512);
					istr.append("\n////////////////////////////////\n");
					istr.append("// CUDA Device Initialization //\n");
					istr.append("////////////////////////////////\n");
					istr.append("int deviceCount;\n");
					if( opt_doNotUseCUTIL ) {
						istr.append("cudaGetDeviceCount(&deviceCount);\n");
						istr.append("if (deviceCount == 0) {\n");
						istr.append("\tfprintf(stderr, \"cutil error: no devices supporting CUDA.\\n\");\n");
						istr.append("\texit(1);\n");
						istr.append("}\n");
						istr.append("int dev = 0;\n");
						istr.append("cudaDeviceProp deviceProp;\n");
						istr.append("cudaGetDeviceProperties(&deviceProp, dev);\n");
						istr.append("\tfprintf(stderr, \"Using device %d: %s\\n\", dev, deviceProp.name);\n");
						istr.append("cudaSetDevice(dev);\n\n");
					} else {
						istr.append("CUDA_SAFE_CALL_NO_SYNC(cudaGetDeviceCount(&deviceCount));\n");
						istr.append("if (deviceCount == 0) {\n");
						istr.append("\tfprintf(stderr, \"cutil error: no devices supporting CUDA.\\n\");\n");
						istr.append("\texit(EXIT_FAILURE);\n");
						istr.append("}\n");
						istr.append("int dev = 0;\n");
						istr.append("cudaDeviceProp deviceProp;\n");
						istr.append("CUDA_SAFE_CALL_NO_SYNC(cudaGetDeviceProperties(&deviceProp, dev));\n");
						istr.append("\tfprintf(stderr, \"Using device %d: %s\\n\", dev, deviceProp.name);\n");
						istr.append("CUDA_SAFE_CALL(cudaSetDevice(dev));\n\n");
					}
				    ...
				    for( Statement rstmt : return_list ) {
				    	...
				    	if( (rParent == null) || !(rParent instanceof CompoundStatement) ) {
				    		Tools.exit("[ERROR in CUDAInitializer()] can't find a parent statment" +
				    				"of a return statement, "  + rstmt);
				    	}
				    	...
				    	flushStmt =(Statement)stmt2.clone();
				    	FlushStmtList.add(flushStmt);
				    	//rParent.addStatementBefore(rstmt, (Statement)stmt2.clone());
				    	rParent.addStatementBefore(rstmt, flushStmt);
				    	rParent.addStatementBefore(rstmt, (Statement)stmt3.clone());
				    }
				    ////////////////////////////////////////////////////////////
				    ...
				    if( return_list.size() == 0 ) {
				    	...
				    	mainBody.addStatement((Statement)stmt3.clone());
				    }
				}
			}
			...
			CUDAHeaderEndMap.put(tu, lastCudaDecl);
			if( main_TU ) {
	 ...
	 * 
	 */
	public static void convParRegionToKernelFunc()
	{
		int numOfKernelRegions = 0;
		c2gMemTr = new HashMap();
		g2cMemTr = new HashMap();
		c2gMap = new HashMap();
		freedSymMap = new HashMap<Symbol, Statement>();
		...
		// For consistant output generation, proc_list is sotred.
		//for (Procedure proc : proc_list)
	    TreeMap<String, Procedure> sortedMap = new TreeMap<String, Procedure>();
		for (Procedure proc : proc_list){
			String procName = proc.getSymbolName();
			sortedMap.put(procName, proc);
		}
		Collection<Procedure> sortedSet = sortedMap.values();
		for (Procedure proc : sortedSet)
		{
			...
			for ( OmpAnnotation annot : omp_annots )
			{
				...
				if( eligibility == 0 ) {
					...
					extractKernelRegion(proc, annot, func_num++);
	 ...
	 */
	private static void extractKernelRegion(Procedure proc, OmpAnnotation annot,
			int func_num) {
		...
		Declaration tLastCudaDecl = CUDAHeaderEndMap.get(tu);
		...
		LinkedHashSet<String> cudaEnclosingLoops = new LinkedHashSet<String>();
		...
		if( cudaAnnots != null ) {
			for( CudaAnnotation cannot : cudaAnnots ) {
				...
				//LinkedHashSet keeps data in insertion-order.
				LinkedHashSet<String> ldataSet = (LinkedHashSet<String>)cannot.get("enclosingloops");
				if( ldataSet != null ) {
					cudaEnclosingLoops.addAll(ldataSet);
				}
				dataSet = (HashSet<String>)cannot.get("multisrccg");
				if( dataSet != null ) {
					cudaMultiSrcCGSet.addAll(dataSet);
				}
				dataSet = (HashSet<String>)cannot.get("conditionalsrc");
				if( dataSet != null ) {
					cudaConditionalSrcSet.addAll(dataSet);
				}
			}
		...
		HashMap lRedMap = (HashMap)c2gMap.get("_lRedMap_");
		if( redMap == null ) {
			///////////////////////////////////////////////////////////////////////////
		...
		HashSet<Symbol> NOmpSharedSet = new HashSet<Symbol>();
		HashSet<Symbol> OmpPrivSet = null;
		//HashSet<Symbol> NOmpPrivSet = new HashSet<Symbol>();
		HashSet<Symbol> OmpFirstPrivSet = null;
		//HashSet<Symbol> NOmpFirstPrivSet = new HashSet<Symbol>();
		HashSet<Symbol> OmpLastPrivSet = null;
		HashSet<Symbol> OmpThreadPrivSet = null;
		HashSet<Symbol> NOmpThreadPrivSet = new HashSet<Symbol>();
		HashSet<Symbol> OmpCopyinSet = null;
		//HashSet<Symbol> NOmpCopyinSet = new HashSet<Symbol>();
		...
		Statement enclosingLoop = null;
		if( !cudaMultiSrcCGSet.isEmpty() ) {
			Traversable tt = region.getParent();
			while (!(tt instanceof Loop) && !(tt instanceof Procedure) ) {
				tt = tt.getParent();
			}
			if( tt instanceof Loop ) {
				enclosingLoop = (Statement)tt;
			}
		}
		...
		int MemTrOptOnLoopsLevel = 0;
		Expression firstItrCondExp = null;
		Expression lastItrCondExp = null;
		if( opt_MemTrOptOnLoops ){
			LinkedList resultList = memTrOptOnLoops(region, cudaEnclosingLoops);
			MemTrOptOnLoopsLevel = resultList.size();
			if( MemTrOptOnLoopsLevel > 0 ) {
				if( MemTrOptOnLoopsLevel == 1 ) { //All local loops.
					refstmt1 = (Statement)resultList.get(0);
				} else { //Enclosing loops are in one of parent procedures.
					firstItrCondExp = (Expression)resultList.get(0);
					lastItrCondExp = (Expression)resultList.get(1);
				}
			}
		} else {
			MemTrOptOnLoopsLevel = 0;
		}
		if( (MemTrOptLevel > 0) && (MemTrOptOnLoopsLevel == 0) ) {
...
/*		if( gridSizeNotChanged ) {
		...
		////////////////////////////////
		List redData = reductionTransformation(proc, region, redMap, lRedMap, call_to_new_proc,
		...
		if( redVarMap.containsKey(parentRegion) ) {
			redItemSet.addAll( (HashSet<Symbol>)redVarMap.get(parentRegion));
		}
		
		//Keep shared variable to CUDA constant variable mapping.
		constMap = new HashMap<Symbol, Symbol>();
		...
		if (OmpSharedSet != null) {
			...
			for( Symbol shared_var : OmpSharedSet ) {
				String symName = shared_var.getSymbolName();
				sortedMap.put(symName, shared_var);
			}
			Collection<Symbol> sortedSet = sortedMap.values();
			for( Symbol shared_var : sortedSet ) {
				boolean isStruct = false;
				if( proc.containsSymbol(shared_var) ) {
					isStruct = SymbolTools.isStruct(shared_var, proc);
				} else {
					isStruct = SymbolTools.isStruct(shared_var, (Traversable)shared_var);
				}
				boolean isLocal = SymbolTools.isLocal(shared_var);
				if( !isLocal ) {
					if( proc.containsSymbol(shared_var) && SymbolTools.isScalar(shared_var) && 
							!SymbolTools.isPointer(shared_var)) {
						isLocal = true;
					}
				}
				////////////////////////////////////////////////////////////////////////////////////
				...
				if( shared_var instanceof VariableDeclarator ) {
					...
					// FIXME: if the variable is struct type, it may cause kernel parameter space //
					// overflow; for now, struct variables are not passed as kernel parameters.   //
					////////////////////////////////////////////////////////////////////////////////
					if( cudaSharedROSet.contains(cloned_ID.getName()) && !isStruct ) {
						...
						call_to_new_proc.addArgument(new Identifier((VariableDeclarator)shared_var));
						
						// Replace the instance of shared variable with the new gpu_var.
						IRTools.replaceAll((Traversable) region, cloned_ID, gpu_var);
						
						tempMap.put(new Identifier(shared_var), gpu_var.clone());
						
						continue;
					}
					
					if( cudaConstantSet.contains(cloned_ID.getName()) ) {
						/////////////////////////////////////////////////////////////////////////
						// Constant reference is declared in a TranslationUnit and can be used // 
						// by multiple procedures existing in the same TranslationUnit.        //
						// (cf. c2gMap is reset for each procedure.)                           //
						/////////////////////////////////////////////////////////////////////////
						// Check whether the enclosing TranslationUnit contains the constant //
						// reference of this shared variable.                                //
						///////////////////////////////////////////////////////////////////////
						List symInfo = AnalysisTools.findOrgSymbol(shared_var, proc);
						Symbol orgSym = null;
						Traversable parentTable = null;
						if( symInfo.size() == 2 ) {
							orgSym = (Symbol)symInfo.get(0);
							parentTable = (Traversable)symInfo.get(1);
						}
						...
						VariableDeclaration const_decl = null;
						Identifier constRefID = null;
						for (Symbol sym : symbolset)
						{
							if( sym.getSymbolName().compareTo(str.toString()) == 0 ) {
								const_decl = (VariableDeclaration)((Declarator)sym).getParent();
								if (const_decl != null) {
									break;
								}
							}
						}
						VariableDeclarator constRef_declarator = null;
						if( const_decl == null ) {
							constRef_declarator = new VariableDeclarator(cloned_declarator.getSpecifiers(), 
									new NameID(str.toString()), cloned_declarator.getTrailingSpecifiers());
							...
							Declaration constRef_decl = new VariableDeclaration(constspecs,
									constRef_declarator);
							constRefID = new Identifier(constRef_declarator);
							///////////////////////////////////////////////////////////////////////////////
							//DEBUG: it seems that the constant declaration should be put at the end of  //
							//last CUDA-related declaration, since the first declaration in a translation//
							//unit will be the first part of CUDA-related head macros.                   //
							///////////////////////////////////////////////////////////////////////////////
							//tu.addDeclarationAfter(tu.getFirstDeclaration(), constRef_decl);
							tu.addDeclarationAfter(tLastCudaDecl, constRef_decl);
							tLastCudaDecl = constRef_decl;
							CUDAHeaderEndMap.put(tu, constRef_decl);
						} else {
							constRef_declarator = (VariableDeclarator)const_decl.getDeclarator(0);
							constRefID = new Identifier(constRef_declarator);
						}
						//Put shared variable to CUDA constant variable mapping.
						constMap.put(shared_var, constRef_declarator);

						// Replace the instances of shared variable with constant variable.
						IRTools.replaceAll((Traversable) region, cloned_ID, constRefID);
						...
						// Ex: CUDA_SAFE_CALL(cudaMemcpyToSymbol(const__b, b, gpuBytes)); 
						FunctionCall memCopy_call = new FunctionCall(new NameID("cudaMemcpyToSymbol"));
						List<Expression> arg_list2 = new ArrayList<Expression>();
						arg_list2.add((Identifier)constRefID.clone());
						if( SymbolTools.isScalar(shared_var) && !SymbolTools.isPointer(shared_var)) {
						...
						if( opt_doNotUseCUTIL ) {
							memCopy_stmt = new ExpressionStatement(memCopy_call);
						} else {
							FunctionCall safe_call = new FunctionCall( new NameID("CUDA_SAFE_CALL"));
							safe_call.addArgument(memCopy_call);
							memCopy_stmt = new ExpressionStatement(safe_call);
						}
						
						/////////////////////////////////////////////////////////////////////////
						//Find memory copy points.                                             //
						/////////////////////////////////////////////////////////////////////////
						boolean isGlobal = false;
						boolean isPProcLocal = false;
						if( parentTable != null ) {
							if( parentTable instanceof TranslationUnit ) {
								isGlobal = true;
							} else if ( parentTable instanceof Procedure ) {
								isPProcLocal = true;
							}
						}
							
						boolean isInALoop = false;
						Traversable memCpyPoint = null;
						boolean lastDefFound = false;
						Symbol cSym = shared_var;
						/////////////////////////////////////////////////////////////////////////
						//If step == 0, check memory copy point before the parallel region.    //
						//Else if step == 1, check memory copy point after the parallel region //
						//     This second step is needed if the parallel region is in a loop. //
						/////////////////////////////////////////////////////////////////////////
						//FIXME: in step 0, if the shared variable is modified in a previous   //
						//kernel function, it can not be detected by Tools.ipaIsDefined(),     //
						//since corresponding device variable instead of the shared variable   //
						...
						while (step < 2) {
							...
							memCpyPoint = null;
							lastDefFound = false;
							Traversable child = region;
							Traversable pp = child.getParent();
							while ( !(pp instanceof CompoundStatement) && !(pp instanceof Procedure) ) {
								child = pp;
								pp = pp.getParent();
							}
							while( pp instanceof CompoundStatement ) {
								CompoundStatement cpStmt = (CompoundStatement)pp;
								List<Traversable> children = cpStmt.getChildren();
								if( step == 0 ) {
									beginIdx = 0;
									endIdx = Tools.indexByReference(children, child) -1;
									refIdx = endIdx;
								} else {
									beginIdx = Tools.indexByReference(children, child) +1;
									endIdx = children.size()-1;
									refIdx = endIdx;
								}
								while( endIdx >= beginIdx ) {
									child = children.get(endIdx);
									if( AnalysisTools.ipaIsDefined(cSym, child)) {
										memCpyPoint = child;
										lastDefFound = true;
										break;
									} else if (child instanceof ExpressionStatement) {
										Expression cExp = ((ExpressionStatement)child).getExpression();
										if( cExp instanceof KernelFunctionCall ) {
											VariableDeclarator gpuSym = (VariableDeclarator)c2gMap.get(cSym);
											if( gpuSym != null ) {
												if( AnalysisTools.ipaIsDefined(gpuSym, child)) {
													//Find a cudaMemcpy() which copies written data back to CPU.
													int tIdx;
													NameID cudaMemcpy = new NameID("cudaMemcpy");
													NameID cudaMemcpy2D = new NameID("cudaMemcpy2D");
													Traversable newMCpyPoint = null;
													tIdx = endIdx+1;
													while( tIdx <= refIdx ) {
														Traversable tC = children.get(tIdx);
														if( IRTools.containsSymbol(tC, cSym) ) {
															List<FunctionCall> fList = IRTools.getFunctionCalls(tC);
															if( fList != null ) {
																for(FunctionCall fCall : fList) {
																	Expression fName = fCall.getName();
																	if(fName.equals(cudaMemcpy) || fName.equals(cudaMemcpy2D)) {
																		newMCpyPoint = tC;
																		break;
																	}
																}
													...
													if( newMCpyPoint == null ) {
														String constflag;
														if( SymbolTools.isScalar(shared_var) ) {
															constflag = "-shrdSclrCachingOnConst";
														} else {
															constflag = "-shrdArryCachingOnConst";
														}
														Tools.exit("[ERROR in extractKernelRegion()] Shared variable, " +
																cloned_ID.getName() + ", is included in the CUDA constant clause of the following parallel reion, " +
																		"but current translator can not handle this correctly; either remove "+ constflag +
																		" commandline option or add noconstant("+cloned_ID.getName()+") clause to the parallel region:\n" +
																				region + "\n");
													} else {
														memCpyPoint = newMCpyPoint;
														lastDefFound = true;
														break;
													}
												}
								...
								if( step == 0 ) {
									...
									do {
										...
										if( pp instanceof Loop ) {
											if( memCpyPoint != null ) {
												if( lastDefFound && (memCpyPoint != null) ) { 
													Traversable mp = memCpyPoint.getParent();
													/*
													while( !(mp instanceof CompoundStatement) ) {
														mp = mp.getParent();
													}
													*/
													((CompoundStatement)mp).addStatementAfter((Statement)memCpyPoint, memCopy_stmt.clone());
													((CompoundStatement)mp).addStatementAfter((Statement)memCpyPoint, gpuBytes_stmt.clone());
												}
												memCpyPoint = null;
												lastDefFound = false;
											}
							...
							if( isGlobal || isPProcLocal ) {
								//FIXME: We can search interprocedurally, but not implemented yet.
							}
							if( step == 0 ) {
								if( memCpyPoint == null ) {
									memCpyPoint = IRTools.getFirstNonDeclarationStatement(child);
									pp = child;
									if( isInALoop ) {
										step = 1;
									} else {
								...
								if( memCpyPoint == null ) {
									Tools.exit("[ERROR in extractKernelRegion()] Error occured while handling cuda constant clause.");
								} else {
									if( lastDefFound ) {
										((CompoundStatement)pp).addStatementAfter((Statement)memCpyPoint, memCopy_stmt);
										((CompoundStatement)pp).addStatementAfter((Statement)memCpyPoint, gpuBytes_stmt);
									} else {
										((CompoundStatement)pp).addStatementBefore((Statement)memCpyPoint, gpuBytes_stmt);
										((CompoundStatement)pp).addStatementBefore((Statement)memCpyPoint, memCopy_stmt);
									}
									if( pp.equals(region.getParent()) ) {
										num_cudastmts-=2;
									}
								}
								if( step == 0 ) {
						...
						continue;
					} //End of "if( cudaConstantSet.contains(cloned_ID.getName()) ) {"
						...
						if( symInfo.size() == 0 ) {
							...
							if( gC2GMap.containsKey(orgSym) ) {
								...
								if( tu != main_TrUnt ) {
									...
									if( !cloned_decl.getSpecifiers().contains(Specifier.EXTERN) ) {
										extended_clonedspecs.add(Specifier.EXTERN);
									}
									extended_clonedspecs.addAll(cloned_decl.getSpecifiers());
									gpu_decl = new VariableDeclaration(extended_clonedspecs, 
										gpu_declarator);
									tu.addDeclarationAfter(tLastCudaDecl, gpu_decl);
									tLastCudaDecl = gpu_decl;
									CUDAHeaderEndMap.put(tu, gpu_decl);
								}
								gpu_var = new Identifier(gpu_declarator);
					...
					if( (refstmt != region) && c2gMap_added ) {
						...
						while( currIndex >= 0 ) {
							...
							if( !kernelCallStmtSet.contains(currStmt)) {
								Set<Symbol> tDefSet = DataFlowTools.getDefSymbol(currStmt);
...
//							}
						...
						if( cudaRegisterSet.contains(cloned_ID.getName()) && !isStruct ) {
							useRegister = true;
						}
						...
						VariableDeclarator pointerV_declarator = scalarVariableConv(cloned_declarator, new_proc, region,
								useRegister, useSharedMemory, ROData);
						pointer_var = new Identifier(pointerV_declarator);
						// Add new shared symbol to omp shared set
						NOmpSharedSet.add(pointerV_declarator);
						...
						// cudaMallocPitch() is used only for 2 dimensional arrays, which are not cached on CUDA constant memory.
						if( dimsize == 2 ) {
							if( cudaConstantSet.contains(cloned_ID.getName()) ) {
								use_MallocPitch = false;
							} else {
...
/*						if( cudaSharedSet.contains(cloned_ID.getName())) {
						...
						if( use_TextureMemory ) {
							...
							if( texture_decl == null ) {
								...
								///////////////////////////////////////////////////////////////////////////////
								//DEBUG: it seems that the texture declaration should be put at the end of   //
								//last CUDA-related declaration, since the first declaration in a translation//
								//unit will be the first part of CUDA-related head macros.                   //
								///////////////////////////////////////////////////////////////////////////////
								//tu.addDeclarationAfter(tu.getFirstDeclaration(), textureRef_decl);
								tu.addDeclarationAfter(tLastCudaDecl, textureRef_decl);
								tLastCudaDecl = textureRef_decl;
								CUDAHeaderEndMap.put(tu, textureRef_decl);
							} else {
							...
							if( insertMalloc2 ) {
								...
								if( insertGMalloc == 1 ) {
									...
									if( tu != main_TrUnt ) {
										...
										List<Specifier> extended_specs = new LinkedList<Specifier>();
										extended_specs.add(Specifier.EXTERN);
										extended_specs.add(CUDASpecifier.SIZE_T);
										pitch_decl = new VariableDeclaration(extended_specs, 
												pitch_declarator);
										pitch_var = new Identifier(pitch_declarator);
										tu.addDeclarationAfter(tLastCudaDecl, pitch_decl);
										tLastCudaDecl = pitch_decl;
										CUDAHeaderEndMap.put(tu, pitch_decl);
									}
								} else {
...
/*								if( firstCudaStmt == null ) {
						...
						cachedArrayElmts.addAll(arrayCachingOnRegister(region, cudaRegisterSet, cudaRegisterROSet, cloned_declarator));
						...
						if( use_TextureMemory ) {
							...
							for (;;)
							{
								...
								if( arrayID.equals(cloned_ID) ) {
									...
									texAccessCall.addArgument((Expression)aAccess.getIndex(0).clone());
						...
						if( dimsize == 1 ) { 
							...
							if( use_MallocPitch ) {
								...
								//////////////////////////////////////////
								// Remove extern specifier if existing. //
								//////////////////////////////////////////
								cloned_pitch_decl.getSpecifiers().remove(Specifier.EXTERN);
								// Change to a pointer type
								// Ex:  "float* b"
								...
								// Add new shared symbol to omp shared set
								NOmpSharedSet.add(cloned_declarator);
							}
						}
					...
					ExpressionStatement malloc_stmt;
					FunctionCall safe_call;
					if( opt_doNotUseCUTIL ) {
						...
						malloc_stmt = new ExpressionStatement(safe_call);
					}
					//if( insertMalloc ) {
					if( insertMalloc2 ) {
...
/*						if( firstCudaStmt == null ) {
					...
					if( use_TextureMemory ) {
						str = new StringBuilder(80);
						str.append("texture__");
						boolean gAllocated = false;
						boolean textureBinded = false;
						VariableDeclarator textureVarDeclr = null;
						if( opt_globalGMalloc && (orgSym != null) ) {
							str.append(orgSym.getSymbolName());
							gAllocated = true;
							textureVarDeclr = (VariableDeclarator)textureRefID.getSymbol();
							if( gC2GMap.get(textureVarDeclr) != null ) {
								textureBinded = true;
							}
						} else {
							str.append(cloned_ID.toString());
						...
						if( insertMalloc2 || ((!gAllocated) && (!c2gMap.containsKey(str.toString()))) ||
								(gAllocated && !textureBinded ) ) {
							c2gMap.put(str.toString(), textureRefID);
							PrintTools.println("Texture Mapping (GPU variable: "
									+gpu_var+" binded to "+textureRefID+")", 3);
							if( gAllocated && !textureBinded ) {
								gC2GMap.put(textureVarDeclr, textureVarDeclr);
							}
							///////////////////////////////////////////////////////////////////
							...
							//     }                                            //
							//////////////////////////////////////////////////////
							Expression MemCheckExp = new BinaryExpression((Identifier)cloned_bytes.clone(),
							...
							MemWarningCall.addArgument((Identifier)cloned_bytes.clone());
							...
							textureBindCall.addArgument((Identifier)textureRefID.clone());
							textureBindCall.addArgument((Identifier)gpu_var.clone());
							textureBindCall.addArgument((Identifier)cloned_bytes.clone());
							((CompoundStatement)mallocPoint.getParent()).addStatementBefore(
					...
					if( insertFree2 || insertGFree ) {
						...
						ExpressionStatement cudaFree_stmt;
						if( opt_doNotUseCUTIL ) {
							cudaFree_stmt = new ExpressionStatement(cudaFree_call);
						} else {
							safe_call = new FunctionCall( new NameID("CUDA_SAFE_CALL"));
							safe_call.addArgument(cudaFree_call);
							cudaFree_stmt = new ExpressionStatement(safe_call);
						}
						if( insertGFree ) {
							for( Statement lastFlushStmt : FlushStmtList ) {
								((CompoundStatement)lastFlushStmt.getParent()).addStatementBefore(lastFlushStmt, 
										(Statement)cudaFree_stmt.clone());
							}
						} else {
							...
							freedSymMap.put(shared_var, mallocPoint);
							if( mallocPoint == region ) {
								num_cudastmts-=1;
							}
					...
					if( use_MallocPitch ) {
						if( insertC2GMemTr ) {
							...
							if( (MemTrOptOnLoopsLevel > 1) && opt_globalGMalloc && !isLocal ) {
								Statement condMemCopy_stmt = new IfStatement(firstItrCondExp.clone(), memCopy_stmt);
								((CompoundStatement)region.getParent()).addStatementBefore(region, condMemCopy_stmt);
								if( firstCudaStmt == null ) {
									firstCudaStmt = condMemCopy_stmt;
								}
								num_cudastmts--;
							} else {
								if( cudaMultiSrcCGSet.contains(cloned_ID.getName()) &&
										(enclosingLoop != null) && (refstmt == region) ) {
									((CompoundStatement)enclosingLoop.getParent()).addStatementBefore(enclosingLoop, memCopy_stmt);
									if( firstCudaStmt == null ) {
						...
						if( insertG2CMemTr ) {
							...
							if( (MemTrOptOnLoopsLevel > 1) && opt_globalGMalloc && !isLocal ) {
								Statement condMemCopy_stmt = new IfStatement(lastItrCondExp.clone(), memCopy_stmt);
								((CompoundStatement)region.getParent()).addStatementAfter(region, condMemCopy_stmt);
						...
						if( insertC2GMemTr ) {
							...
							if( !insertMalloc2 || (mallocPoint != refstmt) ) {
								...
								if( (MemTrOptOnLoopsLevel > 1) && opt_globalGMalloc && !isLocal ) {
									...
									if( cudaMultiSrcCGSet.contains(cloned_ID.getName()) &&
											(enclosingLoop != null) && (refstmt == region) ) {
										((CompoundStatement)enclosingLoop.getParent()).addStatementBefore(enclosingLoop, 
												gpuBytesStmt);
									} else {
		...
		///////////////////////////////////////////////////////////////////////////////////
		// Update registerRO and registerRW clauses to contain actually cached elements. //
		///////////////////////////////////////////////////////////////////////////////////
		cudaAnnots = region.getAnnotations(CudaAnnotation.class);
		if( cudaAnnots != null ) {
			HashSet<CudaAnnotation> emptyAnnots = new HashSet<CudaAnnotation>();
			for( CudaAnnotation cannot : cudaAnnots ) {
				HashSet<String> dataSet = (HashSet<String>)cannot.get("registerRO");
				HashSet<String> deleteSet = new HashSet<String>();
				if( dataSet != null ) {
					for( String strElmt : dataSet ) {
						int indx = strElmt.indexOf('[');
						if( (indx != -1) && !cachedArrayElmts.contains(strElmt) ) {
							deleteSet.add(strElmt);
						}
					}
					dataSet.removeAll(deleteSet);
					if( dataSet.isEmpty() ) {
						cannot.remove("registerRO");
					}
				}
				dataSet = (HashSet<String>)cannot.get("registerRW");
				deleteSet.clear();
				if( dataSet != null ) {
					...
					dataSet.removeAll(deleteSet);
					if( dataSet.isEmpty() ) {
						cannot.remove("registerRW");
					}
				}
				if( (cannot.size() == 0) || 
						((cannot.size() == 1) && cannot.containsKey("gpurun")) ) {
					emptyAnnots.add(cannot);
				}
			}
			if( !emptyAnnots.isEmpty() ) {
				cudaAnnots.removeAll(emptyAnnots);
				region.removeAnnotations(CudaAnnotation.class);
				for( CudaAnnotation cannot : cudaAnnots ) {
					region.annotate(cannot);
				}
			}
		}
		...
		if (OmpThreadPrivSet != null) {
			...
			TreeMap<String, Symbol> sortedMap = new TreeMap<String, Symbol>();
			for( Symbol threadPriv_var : OmpThreadPrivSet ) {
				String symName = threadPriv_var.getSymbolName();
				sortedMap.put(symName, threadPriv_var);
			}
			Collection<Symbol> sortedSet = sortedMap.values();
			for( Symbol threadPriv_var : sortedSet ) {
...
/*					boolean insertG2CMemTr = false;
					...
					if( SymbolTools.isPointer(threadPriv_var) ) {
						...
						VariableDeclarator pointerV_declarator = 
							scalarVariableConv2(cloned_declarator, new_proc, region, useRegister, ROData, tempMap);
						pointer_var = new Identifier(pointerV_declarator);
						// Add new threadprivate symbol to the omp threadprivate set.
						NOmpThreadPrivSet.add(pointerV_declarator);
						...
						if( use_MallocPitch ) {
							...
							str.append(cloned_ID.toString());
							///////////////////////////////////////////////////////////////////////////////////
							// DEBUG: Even if insertMalloc is true, the same pitch variable may exist, since //
							// all pitch variables are declared in the same function body.                   //
							// SymbolTools.findSymbol() is not usable, since it searches parent tables if no //
							// symbols is found in the current symbol table.                                 //
							///////////////////////////////////////////////////////////////////////////////////
							Set<Symbol> varSymSet = SymbolTools.getVariableSymbols(procbody);
							if( insertMalloc && !AnalysisTools.containsSymbol(varSymSet, str.toString()) ) {
								// Create a device local variable to keep pitch value
								// Ex: size_t pitch_b;
								// The type of the device symbol should be a pointer type 
								VariableDeclarator pitch_declarator = new VariableDeclarator(new NameID(str.toString()));
						...
						if( use_MallocPitch ) {
							...
							// Add new threadprivate symbol to the omp threadprivate set.
							NOmpThreadPrivSet.add(arrayV_declarator);
						}
					...
					if( insertMalloc ) {
						...
						if( use_MallocPitch ) {
							if( opt_addSafetyCheckingCode ) {
								...
								CompoundStatement ifBody = new CompoundStatement();
								FunctionCall printfCall = new FunctionCall(new NameID("printf"));
								printfCall.addArgument(new StringLiteral("Size (%u) of pitch, "+pitch_var.toString()+
										", is bigger than the maximum size (%d); \\n"));
								printfCall.addArgument((Identifier)pitch_var.clone());
								printfCall.addArgument(new NameID("MAX_PITCH"));
								ifBody.addStatement(new ExpressionStatement(printfCall));
					 ...
					 * Because cuda-related statements are added in reverse order, 
					 * this function call is added first.
					 * FIXME: If two consecutive parallel regions are executed by the same number of threads,
					 * the values of data in the threadprivate objects of threads other than the initial thread
					 * should be guaranteed to persist between two consecutive active parallel regions, but
					 * the following implementation frees them after each kernel returns.
					 * 
					 */
					//if( insertG2CMemTr ) {
					if( insertMalloc ) {
						...
						FunctionCall cudaFree_call = new FunctionCall(new NameID("cudaFree"));
						cudaFree_call.addArgument((Identifier)gpu_var.clone());
						ExpressionStatement free_stmt;
						if( opt_doNotUseCUTIL ) {
							free_stmt = new ExpressionStatement(cudaFree_call);
						} else {
							safe_call = new FunctionCall( new NameID("CUDA_SAFE_CALL"));
							safe_call.addArgument(cudaFree_call);
							free_stmt = new ExpressionStatement(safe_call);
						}
						...
						freedSymMap.put(threadPriv_var,mallocPoint);
					}
		...
		if (OmpFirstPrivSet != null) {
		    ...
		    TreeMap<String, Symbol> sortedMap = new TreeMap<String, Symbol>();
			for( Symbol fpriv_var : OmpFirstPrivSet ) {
				String symName = fpriv_var.getSymbolName();
				sortedMap.put(symName, fpriv_var);
			}
			Collection<Symbol> sortedSet = sortedMap.values();
			for( Symbol fpriv_var : sortedSet ) {
				if( fpriv_var instanceof VariableDeclarator ) {
					...
					if( SymbolTools.isScalar(fpriv_var) && !SymbolTools.isPointer(fpriv_var) ) {
						...
						if( region instanceof ForLoop ) {
							kernelRegion.addStatement(estmt);
						} else {
		...
		if ((OmpPrivSet != null) || (OmpFirstPrivSet != null)) {
			...
			// Remove local procedure symbols from localSymbols. //
		    ...
		    TreeMap<String, Symbol> sortedMap = new TreeMap<String, Symbol>();
			for( Symbol priv_var : PrivSet ) {
				String symName = priv_var.getSymbolName();
				sortedMap.put(symName, priv_var);
			}
			Collection<Symbol> sortedSet = sortedMap.values();
			for( Symbol priv_var : sortedSet ) {
				if( !localSymbols.contains(priv_var) ) {
					...
					if( decl == null ) {
						Tools.exit("[ERROR in extractKernelRegion()] can not find a declaration for the " +
								"local symbol, " + priv_var + ". This may occur if the declaration " +
								"statement exists inside a local block; if so, the programmer should move the " +
								"declaration statement to the beginning of the enclosing function body manually.");
					} else {
...
/*		if( firstCudaStmt2 == null ) {
		...
		if( opt_forceSyncKernelCall || opt_addCudaErrorCheckingCode ) {
			FunctionCall syncCall = new FunctionCall(new NameID("cudaThreadSynchronize"));
			Traversable pp = firstCudaStmt.getParent();
			if( pp instanceof CompoundStatement ) {
				((CompoundStatement)pp).addStatementBefore(firstCudaStmt, new ExpressionStatement(syncCall));
				if( pp == region.getParent() ) {
					num_cudastmts-=1;
				}
			} else {
				Tools.exit(pass_name + "[Error in extractKernelRegion()] the first CUDA statement (" +
						firstCudaStmt + ") does not have a parent!");
			}
		}
		...
		// new_omp_map contains updated Omp shared set and threadprivate set.
		HashMap new_omp_map = new HashMap();
		new_omp_map.put("shared", NOmpSharedSet);
		new_omp_map.put("threadprivate", NOmpThreadPrivSet);
		/////////////////////////////////////////////////////////////// 
...
/*				try {loopbody.removeChild(lsm_stmt); }
			...
			if( !(lb instanceof Literal) || !lb.toString().equals("0") ) {
				biexp3 = new BinaryExpression((Identifier)gtid.clone(), BinaryOperator.ADD, lb);
				assgn = new AssignmentExpression(ivar, 
						AssignmentOperator.NORMAL, biexp3);
			} else {
				assgn = new AssignmentExpression(ivar, 
						AssignmentOperator.NORMAL, gtid.clone());
			}
			...
			deviceFuncTransform((CompoundStatement)loopbody, annot, 
					(List<VariableDeclaration>)new_proc.getParameters(), new_omp_map, 
					cudaSharedSet, cudaSharedROSet, cudaRegisterSet, cudaRegisterROSet);
...
/*					IfStatement ifstmt = new IfStatement((Expression)ploop.getCondition().clone(),
			...
			interProcLoopTransform((CompoundStatement)region, ispaces, annot, 
					(List<VariableDeclaration>)new_proc.getParameters(), tempMap, new_omp_map);
		...
		if( firstMainStmt == region ) {
			//////////////////////////////////////////////////////////////////////////////////////////
			//Current region, which is pointed by firstMainStmt, will be removed from the main body //
			//as a result of this kernel transformation. Therefore, we have to set firstMainStmt to //
			//a new first statement in the main body.                                               //
			//////////////////////////////////////////////////////////////////////////////////////////
			firstMainStmt = IRTools.getFirstNonDeclarationStatement(main.getBody());
		}
	 ...
	 * if the commandline option, localRedVarConf, is set to 1, GPU shared memory
	 * is used for in-block reduction, but this may set a limit on
	 * the applicability of the reduction transformation.
	 * Else if the option is set to 0, GPU global memory is used for in-block
	 * reduction. 
	...
	private static List reductionTransformation(Procedure proc, Statement region, 
			HashMap redMap, HashMap lRedMap, FunctionCall call_to_new_proc, Procedure new_proc,
...
/*		if( reduction_map != null ) {
		...
		for( HashMap reductionMap : redMapList ) {
			...
			for (String ikey : (Set<String>)(reductionMap.keySet())) {
			    ...
			    TreeMap<String, Symbol> sortedMap = new TreeMap<String, Symbol>();
				for( Symbol redSym : redSet ) {
					String symName = redSym.getSymbolName();
					sortedMap.put(symName, redSym);
				}
				Collection<Symbol> sortedSet = sortedMap.values();
				for( Symbol redSym : sortedSet ) {
					if( redSym instanceof VariableDeclarator ) {
						...
						Identifier cloned_ID = new Identifier(cloned_declarator);
						VariableDeclarator gpu_declarator = null;
						VariableDeclarator localRed_declarator = null;
						VariableDeclarator extended_declarator = null;
						Identifier gpu_var = null;
						Identifier localRed_var = null;
						Identifier extended_var = null;
						ArraySpecifier aspec = null;
						int dimsize = 0;
						boolean createLocalRedVar = false;
						if( (localRedVarConf == 0) && SymbolTools.isArray(redSym) ) {
							createLocalRedVar = true;
						}
						...
						if( redMap.containsKey(redSym)) {
							...
							gpu_var = new Identifier(gpu_declarator);
							localRed_declarator = (VariableDeclarator)lRedMap.get(redSym);
							localRed_var = new Identifier(localRed_declarator);
						} else {
							//////////////////////////////////////////////////////////
							...
							//insertMalloc = true;
							///////////////////////////////////////////////////////////////
							// Create a GPU device variable to hold local reduction data //
							// if localRedVarConf == 0 and if the reduction variable is  //
							// array type.                                               //
							// Ex: float *lred__x; //GPU variable for local reduction    //
							///////////////////////////////////////////////////////////////
							if( createLocalRedVar ) {
								str = new StringBuilder(80);
								str.append("lred__");
								str.append(cloned_ID.toString());
								// The type of the device symbol should be a pointer type 
								localRed_declarator = new VariableDeclarator(PointerSpecifier.UNQUALIFIED, 
										new NameID(str.toString()));
								VariableDeclaration localRed_decl = new VariableDeclaration(cloned_decl.getSpecifiers(), 
										localRed_declarator);
								localRed_var = new Identifier(localRed_declarator);
								procbody.addDeclaration(localRed_decl);
								// Add mapping from gpu_declarator to localRed_declarator 
								lRedMap.put(redSym, localRed_declarator);
...
/*							assignex = new AssignmentExpression((Expression)cloned_bytes.clone(),
							...
							if( createLocalRedVar ) {
							/////////////////////////////////////////////////////////////////////////////////////
							// Add malloc size (gpuBytes) statement for local reduction data                   //
						    // Ex: gpuBytes= gpuNumThreads * gpuNumBlocks * (SIZE1 * SIZE2 * sizeof (float));  //
							/////////////////////////////////////////////////////////////////////////////////////
								biexp = new BinaryExpression((Expression)numBlocks.clone(),
										BinaryOperator.MULTIPLY, biexp2.clone());
								assignex = new AssignmentExpression((Identifier)cloned_bytes.clone(),
										AssignmentOperator.NORMAL, new BinaryExpression((Identifier)numThreads.clone(),
												BinaryOperator.MULTIPLY, (Expression)biexp.clone()));
								gpuBytes_stmt2 = new ExpressionStatement(assignex);
							}
							if( insertMalloc ) {
								((CompoundStatement)mallocPoint.getParent()).addStatementBefore(mallocPoint, estmt);
						...
						if( createLocalRedVar && insertMalloc ) {
							malloc_call = new FunctionCall(new NameID("cudaMalloc"));
							...
							castspecs = null;
							assignex = null;
							((CompoundStatement)mallocPoint.getParent()).addStatementBefore(mallocPoint, gpuBytes_stmt2);
							if( firstCudaStmt2 == null ) {
								firstCudaStmt2 = gpuBytes_stmt2;
							}
							if( mallocPoint == region ) {
								num_cudastmts-=1;
							}
						...
						if( createLocalRedVar && insertMalloc ) {
							if( opt_addSafetyCheckingCode ) {
								...
								((CompoundStatement)mallocPoint.getParent()).addStatementAfter(mallocPoint, 
										(Statement)gMemSub_stmt.clone());
								((CompoundStatement)mallocPoint.getParent()).addStatementAfter(mallocPoint, 
										(Statement)gpuBytes_stmt2.clone());
								if( mallocPoint == region ) {
									num_cudastmts-=2;
								}
							}
							// Insert "CUDA_SAFE_CALL(cudaFree(gpu_x));" statement.
							FunctionCall cudaFree_call = new FunctionCall(new NameID("cudaFree"));
							cudaFree_call.addArgument((Identifier)localRed_var.clone());
							ExpressionStatement free_stmt;
							if( opt_doNotUseCUTIL ) {
								free_stmt = new ExpressionStatement(cudaFree_call);
							} else {
		...
		if (OmpSharedSet != null) {
			for( Symbol shared_var : OmpSharedSet ) {
				...
				if( shared_var instanceof VariableDeclarator ) {
					...
					//Check whether corresponding GPU variable is already freed or not.
					Statement mPoint = freedSymMap.get(shared_var);
					if( mPoint != null ) {
						isAlreadyFreed = true;
						Traversable t = (Traversable)refstmt;
					...
					/////////////////////////////////////////
					//FIXME: this may be too conservative. //
					/////////////////////////////////////////
					if( opt_MemTrOptOnLoops && !cudaEnclosingLoops.isEmpty() ) {
						insertG2CMemTr = true;
					}
					 ...
					 */
					boolean insertC2GMemTr = false;
					Set<Symbol> tDefSet = DataFlowTools.getDefSymbol(region);
					Set<Symbol> DefSet = AnalysisTools.getBaseSymbols(tDefSet);
					if( DefSet.contains(shared_var) ) {
						insertC2GMemTr = true;
					}
...
/*	    		  int eligibility = checkKernelEligibility(stmt);
	...
	private static void setKernelConfParameters(SymbolTable global_table, Statement region, Expression iterspace, 
		...
		//////////////////////////////////////////////////////////////////////////////
		// Check whether symbols used in iterspace are changed after firstCudaStmt. //
		// If so, refStmt is conservatively set to region.                          //
		// Otherwise, firstCudaStmt can be safely used as refStmt.                  //
		//////////////////////////////////////////////////////////////////////////////
		Statement refStmt = null;
		Traversable fcParent = firstCudaStmt.getParent();
		Traversable tt = region.getParent();
		if( fcParent != tt ) {
			while( fcParent != tt.getParent() ){
				tt = tt.getParent();
			}
			Set<Symbol> usedSymbols = DataFlowTools.getUseSymbol(iterspace);
			Set<Symbol> defSymbols = DataFlowTools.getDefSymbol(tt);
			usedSymbols.retainAll(defSymbols);
			if( usedSymbols.isEmpty() ) { //No symbols in the iterspace is modified.
				refStmt = firstCudaStmt;
			} else {
				refStmt = region;
			}
		} else {
		...
		CompoundStatement parentStmt = (CompoundStatement)refStmt.getParent();
		...
		TransformTools.addStatementBefore(parentStmt, refStmt, new DeclarationStatement(dimBlock_decl));
		if( region.getParent() == refStmt.getParent() ) {
		...
		if( iterspace instanceof IntegerLiteral ) {
			...
			Expression argExp = Symbolic.divide(floatsize, 
			...
			///////////////////////////////////////////////////////////////////////////////
			List<MinMaxExpression> maxExpList = IRTools.getExpressionsOfType(argExp, MinMaxExpression.class);
			...
			specs.add(Specifier.INT);
			blockSize = new Typecast(specs, ceilfunc);
		}
		...
		parentStmt.addStatementBefore(refStmt, expStmt);
		...
		if( blockSize instanceof IntegerLiteral ) {
			...
			specs = new ArrayList<Specifier>(1);
			specs.add(Specifier.INT);
			blockSize2 = new Typecast(specs, ceilfunc);
		}
		...
		elseBody.addStatement(new ExpressionStatement(asExp));
		parentStmt.addStatementBefore(refStmt, ifStmt);
		if( region.getParent() == refStmt.getParent() ) {
		...
		TransformTools.addStatementBefore(parentStmt, refStmt, new DeclarationStatement(dimGrid_decl));
		if( region.getParent() == refStmt.getParent() ) {
			num_cudastmts--; // This counter does not include statements generated by this O2G translator
		}
		...
		parentStmt.addStatementBefore(refStmt, new ExpressionStatement(asExp));
		if( region.getParent() == refStmt.getParent() ) {
			num_cudastmts--; // This counter does not include statements generated by this O2G translator
		}
		List<Expression> kernelConf = new ArrayList<Expression>();
	...
	private static VariableDeclarator scalarVariableConv( VariableDeclarator targetSym, Procedure new_proc,
			Statement region, boolean useRegister, boolean useSharedMemory, boolean ROData) {
		// Create a parameter Declaration for the kernel function
		// Change the scalar variable to a pointer type 
		...
		if( region instanceof CompoundStatement ) {
			...
			Tools.exit(pass_name + "[ERROR] Unknown region in extractKernelRegion(): "
					+ region.toString());
		}
		if( useSharedMemory || useRegister ) {
			...
			if( useSharedMemory ) {
				...
				// if region is a loop.
				local_var = SymbolTools.getTemp(targetStmt, (Identifier)cloned_ID);
			}
	...
	private static VariableDeclarator scalarVariableConv2(VariableDeclarator targetSym, Procedure new_proc,
			Statement region, boolean useRegister, boolean ROData, HashMap<Identifier, Expression> tempMap) {
		// Create a parameter Declaration for the kernel function
		// Change the scalar variable to a pointer type 
		Identifier cloned_ID = new Identifier(targetSym);
		VariableDeclarator pointerV_declarator = new VariableDeclarator(PointerSpecifier.UNQUALIFIED, 
	 ...
	 */
	private static HashSet<String> arrayCachingOnRegister(Statement region, HashSet<String> cudaRegisterSet,
		...
		if( cudaRegisterSet.isEmpty() ) {
			//PrintTools.println("arrayCachingOnRegister() end", 0);
			return cachedArrayElmts;
		}
		if( region instanceof CompoundStatement ) {
			targetStmt = (CompoundStatement)region;
			isCompoundStmt = true;
		...
		for( ArrayAccess aAccess : aAccessMap.values() ) {
			...
			List<Expression> indexList = aAccess.getIndices();
			...
			if( constantIndices ) {
				...
				if( !isCompoundStmt ) {
					////////////////////////////////////////////////////////////////////////
					// check whether omp-parallel-for loop body contains the array access //
					// more than once.                                                    //
					////////////////////////////////////////////////////////////////////////
				...
				for (;;)
				{
					...
					if( aAccess.equals(tAccess) ) {
						...
						while( t != targetStmt ) {
							while( !(t instanceof ForLoop) && !(t == targetStmt) ) {
								t = t.getParent();
							}
							if( t instanceof ForLoop ) {
								indexSym = ((Identifier)LoopTools.getIndexVariable((ForLoop)t)).getSymbol();
	...
	private static void privateVariableCachingOnSM(Statement region, HashSet<String> cudaSharedSet,
		...
		if( (cudaSharedSet == null) || (OmpPrivSet == null) ) {
			return;
		}
		CompoundStatement targetStmt = null;
		if( region instanceof CompoundStatement ) {
	    ...
	    TreeMap<String, Symbol> sortedMap = new TreeMap<String, Symbol>();
		for( Symbol privSym : OmpPrivSet ) {
			String symName = privSym.getSymbolName();
			sortedMap.put(symName, privSym);
		}
		Collection<Symbol> sortedSet = sortedMap.values();
		for( Symbol privSym : sortedSet ) {
			...
			if( cudaSharedSet.contains(symbolName) ) {
				if( SymbolTools.isScalar(privSym) ) {
					...
					//DEBUG: The original private symbol (privSym) can not be use, since private variable 
					//in the region points to the newly replicated symbol.
					//Traversable t = ((VariableDeclarator)privSym).getParent();
					Declaration privSymDecl = SymbolTools.findSymbol(targetStmt, cloned_ID);
					if( privSymDecl == null ) {
						PrintTools.println("[WARNING in privateVariableCachingOnSM()] Can not find the Declaration" +
								"of the private symbol, " + symbolName + "!", 1);
					} else {
						Traversable t = (Traversable)privSymDecl;
					...
					// Create an extended array type.                                        //
					///////////////////////////////////////////////////////////////////////////
					// If the private array is used as a pointer or partial array, left-most //
					// dimension will be expanded, which may suffer from shared-memory bank  //
					// conflict. (Ex: "float b[BLOCK_SIZE][SIZE1][SIZE2]")                   //
					// Otherwise, right-most dimension will be expanded.                     //
					//     Ex: "float b[SIZE1][SIZE2][BLOCK_SIZE]"                           //
					///////////////////////////////////////////////////////////////////////////
					boolean leftExpansion = false;
					Identifier cloned_ID = new Identifier((VariableDeclarator)privSym);
					DepthFirstIterator iter = new DepthFirstIterator(region);
					for (;;)
					{
						IDExpression IDaccess = null;

						try {
							IDaccess = (IDExpression)iter.next(IDExpression.class);
						} catch (NoSuchElementException e) {
							break;
						}
						if( IDaccess.equals(cloned_ID) ) {
							Traversable p = IDaccess.getParent();
							if( p instanceof ArrayAccess ) {
								ArrayAccess aAccess = (ArrayAccess)p;
								if( aAccess.getArrayName().equals(cloned_ID) &&
										(aAccess.getNumIndices() < dimsize) ) {
									leftExpansion = true;
									break;
								}
							} else if( !(p instanceof Declarator) ){
								leftExpansion = true;
								break;
							}
						}
					...
					//replace b[k][m] with sh__b[k][m][tid] or sh__b[tid][k][m].
					NameID tid = new NameID("threadIdx.x");
					...
					// Replace all instances of the private variable to the extended array variable.
					if( leftExpansion ) {
						/////////////////////////////////////////////////////////////////////////
						// Ex: when b[k] is expanded to sh__[tid][k], "b + k" is replaced with //
						// "sh__b[tid] + k".                                                   //
						/////////////////////////////////////////////////////////////////////////
						tid = new NameID("threadIdx.x");
						ArrayAccess extendedAccess = new ArrayAccess((IDExpression)array_var.clone(), tid.clone());
						IRTools.replaceAll((Traversable) targetStmt, cloned_ID, extendedAccess);	
					} else {
	...
	private static VariableDeclarator scalarReductionConv(BinaryOperator redOp, 
		...
		if( region instanceof CompoundStatement ) {
			...
			targetRegion = (CompoundStatement)((ForLoop)region).getBody();
			if( region.containsAnnotation(OmpAnnotation.class, "parallel")) {
				parallelRegion = (CompoundStatement)targetRegion;
			} else {
	...
	////////////////////////////////////////////////////////////////////////
	// DEBUG: This method is outdated, compared to arrayReductionConv2(). //
	////////////////////////////////////////////////////////////////////////
	...
	private static VariableDeclarator arrayReductionConv2(BinaryOperator redOp, 
		...
		ArraySpecifier aspec = (ArraySpecifier)aspecs.get(0);
		int dimsize = aspec.getNumDimensions();
		boolean	localRedVarOnSM = true;
		if( localRedVarConf == 0 ) {
			localRedVarOnSM = false;
		}
		StringBuilder str = new StringBuilder(80);
		str.append("red__");
		str.append(tarSym.getSymbolName());
		...
		/////////////////////////////////////////////////////////////////////////
		// 1)If localRedVarConf == 1, create an array on the GPU shared memory,//
		// which holds privatized reduction variable.                          //
		// Assumption: GPU threadblock is an one-dimensional array of threads. //
		// Ex: __shared__ float sh__x[SIZE1][SIZE2][BLOCK_SIZE];               //
		// 2)Else if localRedVarConf == 0, create an array on the GPU global   //
		// memory to hold privatized reduction variable.                       //
		// Ex: float sh__x[][SIZE1][SIZE2]                                     //
		/////////////////////////////////////////////////////////////////////////
		edimensions = new LinkedList<Expression>();
		if( !localRedVarOnSM ) {
			edimensions.add(null);
		...
		for (;;)
		{
			...
			if( arrayID.equals(tarSymID) ) {
				if( aAccess.getNumIndices() == dimsize ) {
					...
					//indices.addAll(aAccess.getIndices());
					if( !localRedVarOnSM ) {
						indices.add((Identifier)gtid.clone());
					}
					for( Expression indx : aAccess.getIndices() ) {
						indices.add(indx.clone());
					}
		...
		for( int i=dimsize-1; i>=0; i-- ) {
			...
			loop_init = new ExpressionStatement(assignex);
			condition = new BinaryExpression((Identifier)index_var.clone(),
					BinaryOperator.COMPARE_LT, (Expression)edimensions.get(i+offset).clone());
			step = new UnaryExpression(UnaryOperator.POST_INCREMENT, 
					(Identifier)index_var.clone());
			loop_body = new CompoundStatement();
			if( i == (dimsize-1) ) {
	...
	private static void interProcLoopTransform( CompoundStatement region, List<Expression> ispaces,
			HashMap parMap, List<VariableDeclaration> callerParamList, HashMap<Identifier, Expression> tempMap,
			HashMap newOmpMap) {
		HashMap<Identifier, Expression> localTempMap = new HashMap<Identifier, Expression>();
		HashMap<Expression, Identifier> reverseTempMap = new HashMap<Expression, Identifier>();
		...
		for( FunctionCall calledProc : funcCalls ) {
			...
			if( StandardLibrary.contains(calledProc) ) {
				if( !CudaStdLibrary.contains(calledProc) ) {
					PrintTools.println("[WARNING in interProcLoopTransform()] C standard library function ("+calledProc.getName()+
			...
			if( tProc != null ) {
				PrintTools.println(pass_name + "interProcLoopTransform() analyzes " + tProc.getSymbolName(), 2);
				reverseTempMap.clear();
				/*
				...
				// DEBUG
/*				PrintTools.println("Omp shared set: " + AnalysisTools.symbolsToString(OmpSharedSet, ", "), 0);
				PrintTools.println("Omp threadprivate set: " + AnalysisTools.symbolsToString(OmpThreadPrivSet, ", "), 0);*/
					...
					for( Expression exp : UseSet) {
						...
						if( exp instanceof Identifier ) {
							if( tempMap.containsKey((Identifier)exp) ) {
								...
								reverseTempMap.put(tempExp, (Identifier)exp.clone());
								tempVarUsed = true;
							}
				...
				// DEBUG: the above statement semms to be wrong; use the following:
				Set<Symbol> localSymbols = SymbolTools.getVariableSymbols(tProc.getBody());
				///////////////////////////////////////////////////////////////////////////////////////////////////////
				// DEBUG: Iteration order of HashSet may change over time, and thus each compilation with the same   //
				// input may generate different output code; bad for debugging. To allow reproducible output, sorted //
				// set (TreeSet) is used instead of the original HashSet.                                            //
				...
				for( Symbol ssm : OmpSet ) {
					...
					sortedMap.put(symName, ssm);
				}
				Collection<Symbol> sortedSet = sortedMap.values();
				for( Symbol ssm : sortedSet ) {
...
/*					if( !OmpAnalysis.containsSymbol(argSyms, symName) && 
					...
					if( !AnalysisTools.containsSymbol(oldParamSet, symName) &&
						...
						} else if (!constMap.containsKey(ssm)) {
							VariableDeclarator cparam_declarator = 
								(VariableDeclarator)AnalysisTools.findsSymbol(callerParamSet, symName);
							Identifier var_ID = null;
							if( cparam_declarator != null ) {
								var_ID = new Identifier((VariableDeclarator)cparam_declarator);
							} else {
								var_ID = new Identifier(ssm);
							}
							if( tempMap.containsKey(var_ID) ) {
								gpu_funcCall.addArgument((Expression)tempMap.get(var_ID).clone());
							} else {
				...
				if( !func_exist ) {
					...
					//////////////////////////////////////////////////////////////
					// The new device function is created per translation unit, //
					// and thus it should be a static function.                 //
					//////////////////////////////////////////////////////////////
					if( !return_types.contains(Specifier.STATIC) ) {
						new_proc_ret_type.add(Specifier.STATIC);
					}
					new_proc_ret_type.addAll(return_types);
					body = (CompoundStatement)tProc.getBody().clone();
					///////////////////////////////////////////////////////////////
					// DEBUG: updateAnnotationsInRegion(region) should be called //
					// after the input region is attached to IR tree.            //
					///////////////////////////////////////////////////////////////
						...
						if( arg instanceof Identifier ) {
							...
							if( sm == null ) {
								...
								if( use_MallocPitch && pitchMap.containsKey(sm) ) {
									...
									VariableDeclarator pointV_declarator = null;
									if( isSharedVar ) {
										pointV_declarator = pitchedAccessConv(param_declarator, new_proc, pitch_decl, body);
										pitchMap.put(pointV_declarator, pitch_decl);
									} else if( isTPVar ){
										pointV_declarator = pitchedAccessConv3(param_declarator, new_proc, pitch_decl, body);
										pitchMap.put(pointV_declarator, pitch_decl);
									} else {
										Tools.exit(pass_name + "[ERROR1] interProcLoopTransform() found wrong symbol, "
									...
									if( sm == null ) {
										...
										if( use_MallocPitch && pitchMap.containsKey(sm) ) {
											...
											if( isSharedVar ) {
												...
												Tools.exit(pass_name + "[ERROR2] interProcLoopTransform() found wrong symbol, "
														+ sm.toString() + " in the pitchMap.");
											}
										} else {
											if( isTPVar && SymbolTools.isArray(param_declarator) ) {
				...
				/* 
					...
					///////////////////////////////////////////////////////////////////////////////////////////////////////
					//for( Symbol ssm : OmpSet ) {
				    sortedMap = new TreeMap<String, Symbol>();
					for( Symbol ssm : OmpSet ) {
						String symName = ssm.getSymbolName();
						sortedMap.put(symName, ssm);
					}
					sortedSet = sortedMap.values();
					for( Symbol ssm : sortedSet ) {
						VariableDeclarator symVar = (VariableDeclarator)ssm;
						Identifier ssmID = new Identifier(symVar);
						String symName = ssm.getSymbolName();
/*						if( !OmpAnalysis.containsSymbol(argSyms, symName) && 
						...
						if( !AnalysisTools.containsSymbol(oldParamSet, symName) &&
							...
							if( use_MallocPitch && AnalysisTools.containsSymbol(pitchMap.keySet(),symName) ) {
								...
								VariableDeclarator pointV_declarator = null;
								if( isSharedVar ) {
									pointV_declarator = pitchedAccessConv(symVar, new_proc, pitch_decl, body);
									pitchMap.put(pointV_declarator, pitch_decl);
								} else if( isTPVar ) {
									pointV_declarator = pitchedAccessConv3(symVar, new_proc, pitch_decl, body);
									pitchMap.put(pointV_declarator, pitch_decl);
								}
							} else if (constMap.containsKey(ssm)) {
								// Replace the instances of shared variable with constant variable.
								Identifier cloned_ID = new Identifier(ssm);
								Identifier constRefID = new Identifier(constMap.get(ssm));
								IRTools.replaceAll((Traversable) body, cloned_ID, constRefID);
							} else {
								if( SymbolTools.isScalar(ssm) ) {
									...
									boolean ROData = false;
									boolean isStruct = false;
									if( tProc.containsSymbol(ssm) ) {
										isStruct = SymbolTools.isStruct(ssm, tProc);
									} else {
										isStruct = SymbolTools.isStruct(ssm, (Traversable)ssm);
									}
									if( cudaRegisterSet.contains(symName) && !isStruct) {
										useRegister = true;
									}
									if( cudaRegisterROSet.contains(symName)) {
										ROData = true;
									}
									if( cudaSharedSet.contains(symName)) {
										useSharedMemory = true;
									}
									if( cudaSharedROSet.contains(symName)) {
										ROData = true;
									}
									if( cudaSharedROSet.contains(symName) && !isStruct ) {
										// Create a GPU kernel parameter corresponding to shared_var
										Identifier cloned_ID = new Identifier(ssm);
										VariableDeclarator gpu_declarator = new VariableDeclarator(new NameID(symName));
										List<Specifier> clonedspecs = new ChainedList<Specifier>();
										/////////////////////////////////////////////////////////////////////////////////////
										// CAUTION: VariableDeclarator.getTypeSpecifiers() returns both specifiers of      //
										// its parent VariableDeclaration and the VariableDeclarator's leading specifiers. //
										...
										// pointer specifiers too.                                                         //
										/////////////////////////////////////////////////////////////////////////////////////
										clonedspecs.addAll(ssm.getTypeSpecifiers());
										clonedspecs.remove(Specifier.STATIC);
										///////////////////////////////////////////
										// GPU variables should not be constant. //
										///////////////////////////////////////////
										...
										VariableDeclaration gpu_decl = new VariableDeclaration(clonedspecs, 
												gpu_declarator);
										Identifier gpu_var = new Identifier(gpu_declarator);
										new_proc.addDeclaration(gpu_decl);
										
										// Replace the instance of shared variable with the new gpu_var.
										IRTools.replaceAll((Traversable) body, cloned_ID, gpu_var);
										
										localTempMap.put(new Identifier(ssm), gpu_var.clone());
										
										continue;
									}
									if( isSharedVar ) {
										scalarVariableConv(symVar, new_proc, body, useRegister, useSharedMemory, ROData);
									}
									else if( isTPVar ) {
										scalarVariableConv2(symVar, new_proc, body, useRegister, ROData, localTempMap);
									} else {
					...
					// => symbol pointers of Identifiers are updated at the end of this code block.  //
					///////////////////////////////////////////////////////////////////////////////////
					TransformTools.updateAnnotationsInRegion(new_proc, false);
					...
					if( omp_annots.size() > 0 ) {
						for ( OmpAnnotation fannot : omp_annots ) {
							...
							if( target_stmt instanceof ForLoop ) {
								...
								ispaces.add(iterspace);
								BinaryExpression biexp3 = new BinaryExpression(gtid.clone(), BinaryOperator.ADD, lb.clone());
					...
					/*
					 *  FIXME: Below recursive call seems to be incorrect. 
					 */
					////////////////////////////////////////////////////////////////////
					// DEBUG: below shared/threadprivate set update is needed only if //
					// this method is called for the functions called in a parallel   //
					// region, since omp annotation of the parallel region contains   //
					// old symbols; for the other nested device function calls, this  //
					// update is redundant.                                           //
					////////////////////////////////////////////////////////////////////
					HashMap updatedMap = new HashMap(parMap);
					updatedMap.remove("shared");
					updatedMap.put("shared", newOmpMap.get("shared"));
					updatedMap.remove("threadprivate");
					updatedMap.put("threadprivate", newOmpMap.get("threadprivate"));
					/////////////////////////////////////////////////////////////////////////////////////
					// If temp variables are passed as arguments, replace them with original variables //
					// so that updateOmpMapForCalledFunc() method can detect them.                     //
					/////////////////////////////////////////////////////////////////////////////////////
					List<Expression> orgArgList = new LinkedList<Expression>();
					for( Expression arg : (List<Expression>)gpu_funcCall.getArguments() ) {
						Expression cloned_arg = arg.clone();
						Set<Expression> UseSet = DataFlowTools.getUseSet(cloned_arg);
						for( Expression exp : UseSet) {
							if( reverseTempMap.containsKey(exp) ) {
								Identifier orgID = (Identifier)reverseTempMap.get(exp).clone();
								if( exp.equals(cloned_arg) ) {
									cloned_arg = orgID;
								} else {
									IRTools.replaceAll(cloned_arg, exp, orgID);
								}
							}
						}
						orgArgList.add(cloned_arg);
					}
/*					HashMap new_map = TransformTools.updateOmpMapForCalledFunc(updatedMap, 
							(List<Expression>)gpu_funcCall.getArguments(), new_proc );*/
					HashMap new_map = TransformTools.updateOmpMapForCalledFunc(updatedMap, 
							orgArgList, new_proc );
					// DEBUG Print
					PrintTools.println("[interProcLoopTransform()] new device func:" + new_proc.getSymbolName(),2);
					PrintTools.println("[interProcLoopTransform()] new device func call:" + gpu_funcCall,2);
					HashSet<Symbol> tSet = (HashSet<Symbol>)parMap.get("shared");
					PrintTools.println("[interProcLoopTransform()] old shared set:" + AnalysisTools.symbolsToString(tSet, ","),2);
					tSet = (HashSet<Symbol>)new_map.get("shared");
					PrintTools.println("[interProcLoopTransform()] new shared set:" + AnalysisTools.symbolsToString(tSet, ","),2);
					tSet = (HashSet<Symbol>)parMap.get("threadprivate");
					PrintTools.println("[interProcLoopTransform()] old threadprivate set:" + AnalysisTools.symbolsToString(tSet, ","),2);
					tSet = (HashSet<Symbol>)new_map.get("threadprivate");
					PrintTools.println("[interProcLoopTransform()] new threadprivate set:" + AnalysisTools.symbolsToString(tSet, ","),2);
					PrintTools.println("", 2);
					
					interProcLoopTransform(body, ispaces, new_map, (List<VariableDeclaration>)new_proc.getParameters(),
							localTempMap, new_map);
				...
				////////////////////////////////////////////////////////////////////////////////////////
				// DEBUG: tempMap should not be reset here, since it may be used for remaining called //
				// functions. It should be reset after all called functions are visited.              //
				////////////////////////////////////////////////////////////////////////////////////////
				//tempMap.clear();
				PrintTools.println(pass_name + "interProcLoopTransform() finishes  the analysis on " + tProc.getSymbolName(), 2);
			}
		}
		////////////////////////////////////////////////////////////////////////////////////////////
		// DEBUG: While each called function is cloned, new entries may be added to the tempMap,  //
		// and then these new entries should not be deleted from the tempMap if there exists a    //
		// function called in a function called in a parallel region.                             //
		// ==> Use localTempMap to contain the new entries, and thus we can reset tempMap safely. //
		////////////////////////////////////////////////////////////////////////////////////////////
		tempMap.clear();
	...
	private static void deviceFuncTransform( CompoundStatement loopbody,
			HashMap parMap, List<VariableDeclaration> callerParamList, HashMap newOmpMap,
			HashSet<String> cudaSharedSet, HashSet<String> cudaSharedROSet,
			HashSet<String> cudaRegisterSet, HashSet<String> cudaRegisterROSet ) {
		HashMap<Identifier, Expression> localTempMap = new HashMap<Identifier, Expression>();
		HashMap<Expression, Identifier> reverseTempMap = new HashMap<Expression, Identifier>();
		...
		for( FunctionCall calledProc : funcCalls ) {
			...
			if( StandardLibrary.contains(calledProc) ) {
				if( !CudaStdLibrary.contains(calledProc) ) {
					PrintTools.println("[WARNING in deviceFuncTransform()] C standard library function ("+calledProc.getName()+
						") is called in a kernel region, but not supported by CUDA runtime system V1.1; " +
						"it may cause compilation error if not inlinable.", 0);
				}
				continue;
			...
			boolean gtidIsUsed = false;
			if( tProc != null ) {
				PrintTools.println(pass_name + " deviceFuncTransform() analyzes " + tProc.getSymbolName(), 2);
				reverseTempMap.clear();
				/*
				 * Create a new device function by replicating the procedure, tProc.
				 */
				...
				Set<Symbol> accessedSymbols = SymbolTools.getAccessedSymbols(tProc.getBody());
				//Set<Symbol> localSymbols = DataFlowTools.getDefSymbol(tProc.getBody());
				// DEBUG: the above statement seemst to be wrong; use the following:
				Set<Symbol> localSymbols = SymbolTools.getVariableSymbols(tProc.getBody());
				///////////////////////////////////////////////////////////////////////////////////////////////////////
				// DEBUG: Iteration order of HashSet may change over time, and thus each compilation with the same   //
				// input may generate different output code; bad for debugging. To allow reproducible output, sorted //
							...
							if( sm == null ) {
								...
								if( use_MallocPitch && pitchMap.containsKey(sm) ) {
									...
									if( isSharedVar ) {
										...
										Tools.exit(pass_name + "[ERROR1] deviceFuncTransform() found wrong symbol, "
												+ sm.toString() + " in the pitchMap.");
									}
								} else {
									if( isTPVar && SymbolTools.isArray(param_declarator) ) {
									...
									if( sm == null ) {
										...
										if( use_MallocPitch && pitchMap.containsKey(sm) ) {
											...
											if( isSharedVar ) {
												...
												Tools.exit(pass_name + "[ERROR2] deviceFuncTransform() found wrong symbol, "
														+ sm.toString() + " in the pitchMap.");
											}
										} else {
											if( isTPVar && SymbolTools.isArray(param_declarator) ) {
							...
							if( use_MallocPitch && AnalysisTools.containsSymbol(pitchMap.keySet(),symName) ) {
								...
								if( SymbolTools.isScalar(ssm) ) {
									...
									if( isSharedVar ) {
										...
										scalarVariableConv2(symVar, new_proc, body, true, false, localTempMap);
									} else {
										Tools.exit(pass_name + "[ERROR] interProcLoopTransform() found unknown symbol, "
												+ ssm.toString());
									}
...
/*					HashMap new_map = TransformTools.updateOmpMapForCalledFunc(updatedMap, 
					...
					HashMap new_map = TransformTools.updateOmpMapForCalledFunc(updatedMap, 
							orgArgList, new_proc );
					//////////////////////////////////////////////////////////////////////////////
					//FIXME: cudaSharedSet, cudaSharedROSet, cudaRegisterSet, cudaRegisterROSet //
					//should be updated too.                                                    //
					//////////////////////////////////////////////////////////////////////////////
					deviceFuncTransform(body, new_map, 
							(List<VariableDeclaration>)new_proc.getParameters(), new_map, 
							cudaSharedSet, cudaSharedROSet, cudaRegisterSet, cudaRegisterROSet);
					SymbolTools.linkSymbol(new_proc);
				}
				/*
				...
				calledProc.swapWith(gpu_funcCall);
				//tempMap.clear();
				PrintTools.println(pass_name + " deviceFuncTransform() finishes the analysis on " + tProc.getSymbolName(), 2);
			}
		}
		tempMap.clear();
	}
	...
	/**
	 * If the input parallel region is enclosed by loops out of the procedure containing the input
	 * parallel region, add index variable, lower bound, and upper bound of each encloing loop as
	 * function parameters and return first-iteration-checking expression and the last-iteration-
	 * checking expression in the resultList.
	 * If the parallel region is enclosed by local loops, return the outmost loop in the resultlist 
	 * 
	 * @param region input parallel region
	 * @param cudaEnclosingLoops a list of index variables of loops containing the parallel region
	 * @return resultlist if the size == 1, it contains the outmost local loop containing the
	 *                    paralllel region
	 *                    else if the size == 2, it contains the first-iteraioin-checking expression
	 *                    and the last-iteration-checking expression
	 *                    else size == 0 which means error.
	 */
	private static LinkedList memTrOptOnLoops(Statement region, LinkedHashSet<String> cudaEnclosingLoops) {
		///////////////////////////////////////////////////////////////////////////////////////////
		// cuda enclosingloops(list) contains a list of index variables containing the region    //
		// interprocedurally.                                                                    //
		// If cuda enclosingloops(list)exists, the following map keeps String to ForLoop mapping //
		// where the index variable name of the ForLoop is the key string.                       //
		// FIXME: In enclosingloops(list), index variables should have distinct names.           //
		///////////////////////////////////////////////////////////////////////////////////////////
		HashMap<String, ForLoop> enLoopMap = new HashMap<String, ForLoop>();
		HashMap<String, LinkedList<FunctionCall>> fCallMap = new HashMap<String, LinkedList<FunctionCall>>();
		Expression firstItrExp = null;
		Expression lastItrExp = null;
		Statement refstmt1 = null;
		LinkedList resultList = new LinkedList();
		if( (cudaEnclosingLoops != null) && (!cudaEnclosingLoops.isEmpty()) ) {
			List<FunctionCall> funcCallList = IRTools.getFunctionCalls(program);
			boolean allLocalLoops = true;
			boolean errorOccurred = false;
			String outmostInd = null;
			for( String indexID : cudaEnclosingLoops ) {
				if( outmostInd == null ) {
					outmostInd = indexID;
				}
				LinkedList<FunctionCall> fList = new LinkedList<FunctionCall>();
				fCallMap.put(indexID, fList);
				//Find an enclosing loop whose index variable is indexID.
				boolean foundLoop = false;
				Traversable t = region.getParent();
				while( !foundLoop && !(t instanceof TranslationUnit) ) {
					while( !(t instanceof Procedure) ) {
						if( t instanceof ForLoop ) {
							Expression indExp = LoopTools.getIndexVariable((Loop)t);
							if( (indExp != null) && (indExp.toString().equals(indexID)) ) {
								//Found enclosing loop!
								enLoopMap.put(indexID, (ForLoop)t);
								foundLoop = true;
								break;
							}
						}
					...
					Traversable prev_t = t;
					if( t instanceof Procedure ) {
						allLocalLoops = false;
						IDExpression procID = ((Procedure)t).getName();
						int numfCalls = 0;
						for( FunctionCall fCall : funcCallList ) {
							if( procID.equals(fCall.getName()) ) {
								//////////////////////////////////////////////////////////////
								//DEBUG: assume that there exist only one function call for //
								//the Procedure t.                                          //
								//////////////////////////////////////////////////////////////
								if( numfCalls > 0 ) {
									errorOccurred = true;
					...
					if( (t == prev_t) || errorOccurred ) {
						//Can't progress to find enclosing loop; error.
						break;
					}
				}
				if( !foundLoop || errorOccurred ) {
					errorOccurred = true;
					break;
				}
			}
			if( errorOccurred ) { 
				PrintTools.println("[WARNING in memTrOptOnLoops()]Error occurred; " +
						"MemTrOptOnLoops option will be ignored for the current region", 0);
				return resultList;
			} else {
				if( allLocalLoops ) {
					refstmt1 = enLoopMap.get(outmostInd);
					resultList.add(refstmt1);
				} else {
					int suffix = 0;
					for( String indexID : cudaEnclosingLoops ) {
						LinkedList<FunctionCall> fList = fCallMap.get(indexID);
						ForLoop enLoop = enLoopMap.get(indexID);
						Expression indID = LoopTools.getIndexVariable(enLoop);
						Expression LBExp = LoopTools.getLowerBoundExpression(enLoop);
						boolean LBExp_Literal = false;
						if( LBExp instanceof Literal ) {
							LBExp_Literal = true;
						}
						Expression UBExp = LoopTools.getUpperBoundExpression(enLoop);
						boolean UBExp_Literal = false;
						if( UBExp instanceof Literal ) {
							UBExp_Literal = true;
						}

						VariableDeclarator IndParamDeclr = null;
						VariableDeclarator LBParamDeclr = null;
						VariableDeclarator UBParamDeclr = null;
						Identifier cIndID = null;
						Expression cLBExp = null;
						Expression cUBExp = null;
						Procedure prev_proc = null;
						while( !fList.isEmpty() ) {
							if( cIndID == null ) {
								cIndID = (Identifier)indID;
							} else {
								cIndID = new Identifier(IndParamDeclr);
							}
							FunctionCall f_call = fList.removeLast();
							if( prev_proc != null ) {
								if( !IRTools.getParentProcedure(f_call).equals(prev_proc) ) {
									Tools.exit("[ERROR in memTrOptOnLoops()] Procedure, "+prev_proc.getSymbolName()+
											", does not have the function call, " + f_call.toString() +
									"\n Please turn off MemTrOptOnLoops option");
								}
							}
							Procedure f_proc = f_call.getProcedure();
							if( f_proc == null ) {
								Tools.exit("[ERROR in memTrOptOnLoops()] Can't find procedure for " +
										"a function call, " + f_call.toString() 
										+ "\n Please turn off MemTrOptOnLoops option");
							}
							/////////////////////////////////////////////////////////////////////
							//Find if declaration statements exist for the original procedure. //
							/////////////////////////////////////////////////////////////////////
							HashSet<VariableDeclaration> funcDeclSet 
							...
							while (Fiter.hasNext())
							{
								...
								for (;;)
								{
									...
									if( procDeclr.getID().equals(f_proc.getName()) ) {
										//Found function declaration.
										VariableDeclaration procDecl = (VariableDeclaration)procDeclr.getParent();
										funcDeclSet.add(procDecl);
										break;
									}
								}
							}
							prev_proc = f_proc;
							List paramList = f_proc.getParameters();
							List argList = f_call.getArguments();
							int list_size = argList.size();
							boolean foundArg = false;
							VariableDeclaration IndParamDecl = null;
							VariableDeclaration LBParamDecl = null;
							VariableDeclaration UBParamDecl = null;
							for( int i=0; i<list_size; i++ ) {
								Expression argExp = (Expression)argList.get(i);
								if( argExp.equals(cIndID) ) {
									foundArg = true;
									IndParamDecl = (VariableDeclaration)paramList.get(i);
									IndParamDeclr = (VariableDeclarator)IndParamDecl.getDeclarator(0);
									break;
								}
							}
							if( !foundArg ) {
								f_call.addArgument(cIndID.clone());
								IndParamDeclr = 
									new VariableDeclarator(new NameID(cIndID.toString() + "_Index" + suffix));
								IndParamDecl = new VariableDeclaration(Specifier.INT, IndParamDeclr);
								f_proc.addDeclaration(IndParamDecl);
								for( VariableDeclaration fDecl : funcDeclSet ) {
									ProcedureDeclarator procDeclr = (ProcedureDeclarator)fDecl.getDeclarator(0);
									procDeclr.addParameter(IndParamDecl.clone());
								}
							}
							if( !LBExp_Literal ) {
								boolean foundLBArg = false;
								Expression tLBExp = null;
								if( LBParamDeclr == null ) {
									tLBExp = LBExp.clone();
								} else {
									tLBExp = new Identifier(LBParamDeclr);
								}
								paramList = f_proc.getParameters();
								argList = f_call.getArguments();
								list_size = argList.size();
								for( int i=0; i<list_size; i++ ) {
									Expression argExp = (Expression)argList.get(i);
									if( argExp.equals(tLBExp) ) {
										foundLBArg = true;
										LBParamDecl = (VariableDeclaration)paramList.get(i);
										LBParamDeclr = (VariableDeclarator)LBParamDecl.getDeclarator(0);
										break;
									}
								}
								if( !foundLBArg ) {
									f_call.addArgument(tLBExp);
									LBParamDeclr = new VariableDeclarator(new NameID(indID.toString()+"_LB" + suffix));
									LBParamDecl = new VariableDeclaration(Specifier.INT, LBParamDeclr);
									f_proc.addDeclaration(LBParamDecl);
									for( VariableDeclaration fDecl : funcDeclSet ) {
										ProcedureDeclarator procDeclr = (ProcedureDeclarator)fDecl.getDeclarator(0);
										procDeclr.addParameter(LBParamDecl.clone());
									}
								}
								cLBExp = new Identifier(LBParamDeclr);
							}
							if( !UBExp_Literal ) {
								boolean foundUBArg = false;
								Expression tUBExp = null;
								if( UBParamDeclr == null ) {
									tUBExp = UBExp.clone();
								} else {
									tUBExp = new Identifier(UBParamDeclr);
								}
								paramList = f_proc.getParameters();
								argList = f_call.getArguments();
								list_size = argList.size();
								for( int i=0; i<list_size; i++ ) {
									Expression argExp = (Expression)argList.get(i);
									if( argExp.equals(tUBExp) ) {
										foundUBArg = true;
										UBParamDecl = (VariableDeclaration)paramList.get(i);
										UBParamDeclr = (VariableDeclarator)UBParamDecl.getDeclarator(0);
										break;
									}
								}
								if( !foundUBArg ) {
									f_call.addArgument(tUBExp);
									UBParamDeclr = new VariableDeclarator(new NameID(indID.toString()+"_UB" + suffix));
									UBParamDecl = new VariableDeclaration(Specifier.INT, UBParamDeclr);
									f_proc.addDeclaration(UBParamDecl);
									for( VariableDeclaration fDecl : funcDeclSet ) {
										ProcedureDeclarator procDeclr = (ProcedureDeclarator)fDecl.getDeclarator(0);
										procDeclr.addParameter(UBParamDecl.clone());
									}
								}
								cUBExp = new Identifier(UBParamDeclr);
							}
						}
						if( cIndID == null ) {
							cIndID = (Identifier)indID;
						...
						if( firstItrExp == null ) {
							firstItrExp = new BinaryExpression(cIndID.clone(), BinaryOperator.COMPARE_EQ, cLBExp.clone());
						} else {
							firstItrExp = new BinaryExpression(firstItrExp, BinaryOperator.LOGICAL_AND, 
									new BinaryExpression(cIndID.clone(), BinaryOperator.COMPARE_EQ, cLBExp.clone()));
						}
						if( lastItrExp == null ) {
							lastItrExp = new BinaryExpression(cIndID.clone(), BinaryOperator.COMPARE_EQ, cUBExp.clone());
						} else {
							lastItrExp = new BinaryExpression(lastItrExp, BinaryOperator.LOGICAL_AND, 
									new BinaryExpression(cIndID.clone(), BinaryOperator.COMPARE_EQ, cUBExp.clone()));
						}
						suffix++;
					}
				}
				if( (firstItrExp != null) && (lastItrExp != null) ) {
					resultList.add(firstItrExp);
					resultList.add(lastItrExp);
				}
			}
		}
		return resultList;
